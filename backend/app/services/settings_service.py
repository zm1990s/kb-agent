...
- "tags": 3-6 个关键词字符串数组
- "content_text": 文件的正文内容，以 Markdown 格式输出（保留标题层级、列表、表格等结构；用于全文检索与展示，尽量完整）。若原文为非中文，须翻译成中文输出
...
请判断：仅凭摘要信息，回答用户问题是否需要查阅某些文档的原文？
- 如果摘要已足够，无需原文。
...
请只输出一个 JSON 对象，二选一：
方案A（摘要已够，无需原文）：{{"mode": "answer", "doc_numbers": [相关文档编号（用于展示来源）]}}
...
{catalog}
{fulltext}
请直接输出 Markdown 格式的回答内容，不要添加任何前缀、后缀或额外说明。"""
