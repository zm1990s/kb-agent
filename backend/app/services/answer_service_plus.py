...
# work/ 是 Codex 内部工作目录；icons/ 图标资源目录；*_unpacked/ 由通用规则覆盖
_INJECTED_PREFIXES = (_SKILL_DIR, _CONTEXT_DIR, "work", "icons")
...
def _is_noise(rel: str) -> bool:
    ...
    if parts[0] in _INJECTED_PREFIXES:
        return True
    # tmp_ 前缀 / *_unpacked 后缀目录（Codex 解压产生的临时工作目录）
    if any(seg.startswith("tmp_") or seg.endswith("_unpacked") for seg in parts[:-1]):
        return True
