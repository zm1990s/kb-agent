...
# 敏感文件名模式（凭据 / 私钥 / 系统账户文件，不限路径深度）
_SENSITIVE_FILE_RE = re.compile(
    r"(?:^|/)\.env(?:\.|$)"                             # .env .env.local 等
    r"|(?:^|/)id_(?:rsa|dsa|ecdsa|ed25519)(?:\.pub)?$"  # SSH 私钥
    r"|\.(?:pem|key|p12|pfx)$"                          # 证书 / PKCS12
    r"|(?:^|/)(?:passwd|shadow|sudoers)$",               # 系统账户文件
    re.IGNORECASE,
)
...
def _is_sensitive(path: Path) -> bool:
    """判断路径是否命中敏感文件黑名单（始终拒绝，不受工作区范围影响）。"""
    s = str(path)
...
def _check_write_content(content: str) -> tuple[bool, str]:
    """检查写入内容中的 env 访问模式和路径安全（黑名单）。返回 (命中, 原因标签)。"""
    if _matches_pattern(content):
        return True, "env_read_pattern_in_written_content"
    for m in _ABS_PATH_RE.finditer(content):
        blocked, label = _path_blocked(m.group(0))
        if blocked:
            return True, f"{label}_in_written_content"
    return False, ""
...
_SENSITIVE_REASON_LABELS = frozenset({
    ...
    "sensitive_file_access_in_written_content",
    "sensitive_file_access_in_edited_content",
    "sensitive_file_access_in_multiedit_content",
    "sensitive_file_access_in_executed_script",
})
_CROSS_STORAGE_REASON_LABELS = frozenset({
    "cross_storage_access",
    "cross_storage_access_in_written_content",
    "cross_storage_access_in_edited_content",
    "cross_storage_access_in_multiedit_content",
