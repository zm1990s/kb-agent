...
- **认证/权限**：邮箱密码注册（域名白名单，DB 维护）、JWT 登录、管理员种子；**用户管理**（启禁用/重置密码/角色）、**用户组**（多维规则自动入组）、**RBAC**（组×模块×读写，admin 绕过）；**自助找回密码**（邮箱 6 位验证码，1 分钟限速、5 次错误锁定、uniform 响应防枚举）。
- **空间**：workspace 隔离；成员按个人 ∪ 用户组授权；空间可删除并打包下载全部文档。
- **文档**：批量/整目录上传（保持结构，单次最大 200 MB）、后台归类（Claude 读原文，支持 Bedrock）、目录树（拖拽移动/改层级/改名）、删除/移动/替换/重命名、全字段展示+自定义列+搜索、文件预览、处理日志与失败重试；**回收站**（删除后软删除，30 天保留期内可恢复，后台每日自动清理过期文件）；文档写操作（上传/删除/重命名/替换/重归类）对空间 **owner / editor** 均开放，不再要求全局 admin。
- **对话**：Agent 式两阶段索引问答（喂全空间索引，按需拉取原文）、SSE 流式工作阶段、气泡 UI + Markdown、多轮上下文、多会话历史、会话命名（AI 摘要生成后通过 `title` SSE 事件实时推送，无竞态；支持用户编辑 + Pin）、**thinking 过程持久化**（存入 sessionStorage，刷新后可折叠查看）、导航离开后恢复状态；聊天侧边栏折叠后改为悬浮按钮（节省页面空间）。
- **新动态**：定时摘要（daily/weekly/biweekly/monthly，可配提示词）、邮件订阅派发（SMTP）、前端展示卡片。
- **数据统计**：用量报表（按天/用户/动作聚合）、下载记录清单、对话记录清单、系统日志查看。
- **系统设置**：品牌配置（Logo/名称）、**AI 引擎 tab**（引擎选择 + 按任务模型配置 + 按任务 HTTP Header 配置）、提示词管理（版本历史/回退）、SMTP 配置、新动态定时配置。4 类 LLM 任务（归类/对话/新动态/会话标题）各自独立选模型；各自可配 HTTP Header 供 Portkey 等网关路由（默认 `{"x-task": "<task_name>"}`）。
...
infra/postgres/  # init.sql（CREATE EXTENSION）+ migrations/（001…020，按序应用，启动时自动执行）
