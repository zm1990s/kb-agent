...
- **系统设置**：品牌配置（Logo/名称）、引擎选择（Claude CLI / OpenAI 兼容引擎）、**按任务模型配置**（归类/对话/新动态/会话标题各自独立选模型）、**按任务 HTTP Header 配置**（4 个任务各自携带可配置 Header，供 Portkey 等网关路由标签，默认 `{"x-task": "<task_name>"}`）、提示词管理（版本历史/回退）、SMTP 配置、新动态定时配置。
