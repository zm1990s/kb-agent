...
- **空间**：workspace 隔离；成员按个人 ∪ 用户组授权。
- **文档**：批量/整目录上传（保持结构）、后台归类（Claude 读原文，支持 Bedrock）、目录树（拖拽移动/改层级/改名）、删除/移动/替换、全字段展示+自定义列+搜索、处理日志与失败重试。
- **对话**：Agent 式索引问答（喂全空间索引，答案带原文下载）、SSE 流式工作阶段、气泡 UI + Markdown、多轮上下文、多会话历史。
- **前端**：Next.js 单端口入口，RBAC 菜单显隐；页面 = 登录/对话/文档/空间管理/用户管理/系统设置。
