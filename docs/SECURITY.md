...
- **本项目处置（现状）**：💡 **已放开 Claude 全部工具**（`--dangerously-skip-permissions`，含 Bash，供 pdftotext 抽取大文档）。缓解：① `ClaudeCliEngine` 用 **argv 列表**传参（非 shell=True），杜绝命令注入；② backend 容器以**非 root 用户**运行；③ 文件路径经 `--add-dir` 授权目录；④ **子进程 env 白名单** + **CLI hook 双层防密钥外泄**（见下 #10）。⚠️ 残余风险：被提示词注入的文档理论上可诱导 CLI 执行任意命令 → 依赖外部 Guardrails（#1）+ 平台可信假设兜底。**上生产前务必确认 Guardrails 在链路上。**
- **子进程环境变量泄露（#10 展开）**：🔒 因 Bash 放开，agent（或注入文档）可跑 `env`/`printenv`/`cat /proc/self/environ`/`os.environ` 读回进程环境里的密钥。两层防护（`app/engine/claude_cli.py` + `backend/claude_hooks/`）：
  1. **精简 env**：`_build_cli_env()` 只白名单透传 `PATH`/`HOME`/`LANG` 等系统变量与 `ANTHROPIC_*`/`AWS_*`/`CLAUDE_*` 前缀认证变量，**剥离** `JWT_SECRET`/`DATABASE_URL`/`SMTP_PASSWORD`/`ADMIN_PASSWORD` 等无关应用密钥——子进程里只剩 Claude 凭据本身，`env` 即便执行也读不到应用密钥。
  2. **PreToolUse hook**（`pre_tool_use.py`）：命令命中 env 读取模式（独立命令 `env`/`printenv`、`/proc/*/environ`、`os.environ`/`getenv`/`process.env`、`$ANTHROPIC/$AWS/$CLAUDE` 引用）→ **deny** + 英文违规话术（反馈给模型转告用户）+ 审计（记录用户/会话/命令）。用 token 边界匹配，不误伤 `venv`/`--env-file`。
  - hook 经引擎 `--settings /app/claude_hooks/settings.json` 确定性注入，覆盖归类/问答/聊天+ 全部 agent 路径；用户标识经子进程 env `KB_AGENT_AUDIT_USER` 传入。审计写 `logs/security-violations.log`（hook 子进程无 DB 连接，故走文件）。
  - ⚠️ **已知边界**：① Claude 凭据本身仍在子进程 env，PreToolUse 靠命令模式匹配拦截，理论上可被绕过（如编码/间接读取）；② **实测 CLI 2.1.197 不支持 PostToolUse `updatedToolOutput`**（无法在模型看到前脱敏工具输出），故未做「按输出值脱敏」兜底层。彻底隔离 Claude 凭据需网关虚拟 key 或出口代理注入（未做，另开任务）。
...
- 🔒 **子进程 env 泄露**：claude 子进程经 `_build_cli_env()` 白名单精简 env + PreToolUse/PostToolUse hook 拦截并按值脱敏 env 读取（详见 #2 展开）。防 agent/注入文档经 Bash 跑 `env` 读回密钥。
...
| 🌐 外置 | #1 提示词注入（外部 Guardrails）、#2 CLI 全工具（非 root + argv + env 白名单 + hook 缓解） |
...
## 本项目已接受/外置的风险（记录在案）
- 💡 **CLI 工具沙箱**：**放开 Claude 全部工具**（`--dangerously-skip-permissions`，含 Bash，用于 pdftotext 等抽取大文件），不做工具/网络/文件系统限制，假设平台可信。缓解措施：backend 容器以**非 root 用户**运行；argv 列表传参避免命令注入；**子进程 env 白名单 + PreToolUse/PostToolUse hook 防密钥外泄**（#2/#10）；提示词注入由外部 Guardrails 负责（#1）。⚠️ 风险：被注入的文档理论上可诱导 CLI 执行任意命令——依赖外部 Guardrails 与平台可信假设兜底。
