...
- **本项目处置**：💡 **工具暂不做沙箱限制，假设平台可信**。⚠️ 注意：即便不做沙箱，`shell=True` + 字符串拼接的命令注入仍应避免（低成本、高收益），建议 ClaudeCliEngine 用 argv 列表传参。
...
- **纵深防御**：上 CSP 头。
- **状态**：⏳ 做前端时落地（当前 MVP 无独立前端）。下载侧的存储型 XSS 已由 #5 的 attachment+nosniff 部分缓解。

### 7. SQL 注入 ⏳
全文检索把 query 转 tsquery 时别拼 SQL，用参数化 / `plainto_tsquery`；ORM 参数绑定。→ M2/M3 检索落地时遵守。

### 8. CORS / CSRF / 安全头 ⏳
- **CORS**：Partner 在外部，不用 `allow_origins=["*"]` + 凭据；白名单具体域名。
- **CSRF**：JWT 放 Authorization header 基本免疫；若改存 cookie 则需防 CSRF。
- **安全头**：CSP、`X-Content-Type-Options: nosniff`、HSTS、`X-Frame-Options`。
...
### 9. 认证与 JWT ⏳（部分 🔒）
- **算法锁定**：校验 JWT 锁死单一算法，拒绝 `alg: none` 与 HS/RS 混淆。
- **过期 / 吊销**：JWT 无状态，账号泄漏过期前踢不掉 → 短过期 + 刷新，或吊销名单/版本号。
- **域名白名单精确匹配**：🔒（DESIGN 已定注册白名单）⚠️ 必须「完整域名 == 白名单项」匹配，**不能 `endswith`**（`endswith("company.com")` 会放行 `fakecompany.com`）；防 `user@company.com.evil.com`、大小写、unicode 同形字、`+` 别名。
- **限速**：登录/注册限速，防暴力破解与账号枚举；bcrypt cost 调够；首个 admin 安全引导。
- **状态**：⏳ M1 认证落地时遵守；域名匹配写法作为 M1-U3b 硬要求。
...
| 时机 | 必做项 |
|------|--------|
| **上线前 P0** | #4 越权校验 🔒、#5 路径穿越+下载 attachment 🔒、#9 JWT 锁算法+白名单精确匹配、#7 SQL 参数化 |
| **接 LLM 时** | #1 注入边界（🌐 外部 Guardrails）、#4 检索先过滤再喂模型 |
| **做前端时** | #6 XSS 转义 + CSP、#8 CORS/安全头 |
| **接 SCM 前** | #3 审批锁定确定性产物 + 审批权限 + 审计（未做好前不开自动下发） |
| **持续** | #10 日志脱敏、#11 限速配额、#12 依赖扫描、#13 审计追踪 |

## 本项目已接受/外置的风险（记录在案）
- 💡 **CLI 工具沙箱**：不做工具/网络/文件系统限制，假设平台可信。（仍建议 argv 传参避免命令注入）
