...
- **纵深防御**：上 CSP 头。
- **状态**：✅ 已落地。前端用 `react-markdown` 渲染 LLM 产物（不解析裸 HTML，无 rehype-raw），从不 `dangerouslySetInnerHTML`；下载侧 attachment+nosniff 双保险。
...
- **安全头**：⏳ CSP / HSTS / X-Frame-Options 尚未全量加（上线前在反代层补）。
...
- **账号枚举**：✅ 登录失败对「邮箱不存在/密码错」返回同一 401。
- **过期 / 吊销**：⏳ JWT 无状态，仅有过期（`JWT_EXPIRE_MIN`）；吊销名单/刷新令牌未做。
- **限速**：⏳ 登录/注册限速未做（上线前在反代层补）。
...
| 状态 | 项 |
|------|--------|
| ✅ 已落地 | #4 越权校验、#5 路径穿越+下载 attachment、#6 XSS(react-markdown)、#7 SQL 参数化、#8 CORS(单端口)、#9 JWT 锁算法+白名单精确匹配+禁用用户 |
| 🌐 外置 | #1 提示词注入（外部 Guardrails）、#2 CLI 全工具（非 root + argv 缓解） |
| ⏳ 上线前补 | #8 CSP/HSTS/X-Frame、#9 登录限速+吊销令牌、#10 日志脱敏、#11 LLM 成本限速/配额、#12 依赖扫描(koi-check/pip-audit)、#13 审计追踪 |
