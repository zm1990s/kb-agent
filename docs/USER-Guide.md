...
普通「对话」和「聊天+」中的会话均支持导出为本地文件。

1. 在对话页面或聊天+ 会话中，点击右上角的「**导出**」按钮。
2. 在弹窗中选择要导出的消息范围（全部消息，或只选某几条）。
3. 选择导出格式：
   - **Markdown (.md)**：纯文本，方便二次编辑或存档；
   - **Word (.docx)**：可在 Word / WPS 等软件中直接打开；
   - **PDF (.pdf)**：带封面标题、代码块高亮、页脚页码，适合正式分享。
4. 点击「导出」，文件自动下载到本地。
...
聊天+ 支持创建**定时任务**，让 Agent 按照预设的时间表自动执行任务，无需手动触发。
...
2. 填写任务信息：
   - **任务名称**：用于标识本条任务。
   - **用户提示词**：每次执行时发给 Agent 的指令（相当于你的提问）。
   - **系统提示词**（可选）：注入的系统级背景说明。
   - **调度类型**：选择触发频率——
     - **每隔 N 分钟**：按固定间隔循环执行；
     - **每天**：选择每天的具体时刻（UTC）；
     - **每周**：选择周几 + 时刻；
     - **每月**：选择每月第几天 + 时刻。
   - **Skill**（可选）：选择要注入的 Skill，让 Agent 具备专项能力。
   - **关联空间**（可选）：为任务指定知识库空间。
3. 点击「**保存**」创建任务；开启「启用」开关后任务即开始按计划执行。

### 15.2 立即运行与查看结果

- 点击任务右侧的「**运行**」按钮可立即触发一次执行，方便测试。
- 每次执行都会自动创建一个新的聊天+ 会话，标题为 `[定时任务]-任务名称`，其中包含本次 Agent 的回复和生成的成果文件。

### 15.3 管理任务

- **启用/停用**：任务列表中的开关控制是否按计划自动运行。
- **编辑**：修改配置后保存，下次运行时间重新计算。
- **删除**：从列表中移除任务，不影响已生成的会话记录。

> **注意**：时刻设置采用 UTC 时间，北京时间（UTC+8）请相应换算。例如每天 8:00（北京时间）= 每天 00:00（UTC）。
